...
export function ExamBuilder({
  ...
  const onSubmit = () => {
    ...
    if (!title.ru.trim()) {
      setError('Укажите название контрольной по-русски')
      return
    }

    const form = new FormData()
    form.set('payload', JSON.stringify({ title, passingScore, questions }))
  ...
  const onDelete = () => {
    if (!window.confirm('Удалить контрольную вместе со всеми попытками студентов?')) return
    startTransition(async () => {
  ...
  return (
    <div className="space-y-4">
      ...
      {saved && <div className="alert alert-success">Контрольная сохранена</div>}

      <div className="card space-y-3" style={{ padding: '1.5rem' }}>
        <LocalizedField
          value={title}
          onChange={setTitle}
          label="Название контрольной *"
          placeholder="Контрольная по модулю 1"
        />

        <div>
          <label className="label">Порог сдачи, %</label>
          <input
            type="number"
            className="input"
            min={1}
          ...
          />
          <div className="hint">Автоматическая оценка. Результат в любом случае уходит учителю на разбор</div>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3" style={{ marginTop: '1rem' }}>
        <button type="button" className="btn btn-primary" onClick={onSubmit} disabled={pending}>
          {pending ? 'Сохраняю…' : 'Сохранить контрольную'}
        </button>
        {hasExisting && (
      ...
      <div className="hint">
        Правильные ответы сохраняются отдельно и в браузер студента не попадают. Результат в любом
        случае отправляется учителю студента (если он есть) на разбор.
      </div>
    </div>
  )
}
